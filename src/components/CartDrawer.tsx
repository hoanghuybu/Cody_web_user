    ...
    <div className={`fixed inset-0 z-[60] ${isCartOpen ? 'pointer-events-auto' : 'pointer-events-none'} `} aria-hidden={!isCartOpen}>
      ...
      >
        <div className="flex items-center justify-between p-4 border-b border-gray-200">
          <h2 className="text-2xl font-bold tracking-wide text-warm-brown">{t('cart.title')}</h2>
          <button onClick={closeCart} aria-label={t('common.close')} className="p-2 rounded-full hover:bg-gray-100">
            <X className="h-5 w-5" />
          </button>
        </div>
        ...
        <div className="flex flex-col h-full min-h-0">
          ...
          <div className="flex-1 min-h-0 overflow-y-auto p-4 space-y-3">
            {items.length === 0 ? (
              <p className="text-center text-gray-500 py-16">{t('cart.drawerEmpty')}</p>
            ) : (
              items.map((item) => {
          ...
          >
            ...
            <div className="flex justify-between text-sm">
              ...
              <span className="font-semibold">{shippingFee === 0 ? t('cart.free') : formatPrice(shippingFee)}</span>
            </div>
            <div className="flex justify-between items-center pt-2 border-t border-gray-200">
              <span className="text-base font-bold">{t('cart.grandTotal')}</span>
